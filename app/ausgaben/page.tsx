...
export default function AusgabenPage() {
  ...
  const [jahresEinnahmen, setJahresEinnahmen] = useState(0)
  ...
  function laden() {
    const alle = ladeBelege()
    setBelege(alle.filter((b) => b.typ === "ausgabe"))
    setJahresEinnahmen(berechneJahresEinnahmen(alle, jahr))
  ...
  return (
    <div className="max-w-6xl mx-auto p-6 space-y-8">
      ...
      <h1 className="text-2xl font-bold">Ausgaben erfassen</h1>
      ...
      <section className="bg-white border rounded-lg p-6">
        <h2 className="text-lg font-semibold mb-4">Neue Ausgabe</h2>
        <BelegForm
          key={formKey}
          typ="ausgabe"
        ...
        <div className="flex justify-between items-center mb-4">
          <div>
            <h2 className="text-lg font-semibold">
              {alleAnzeigen ? "Alle Ausgaben" : `Ausgaben ${MONATE[monat - 1]} ${jahr}`}
            </h2>
            <button
              onClick={() => setAlleAnzeigen((v) => !v)}
              className="text-xs text-blue-600 hover:underline mt-0.5"
            >
              {alleAnzeigen ? "← Nur aktuellen Monat anzeigen" : "Alle Ausgaben anzeigen →"}
            </button>
          </div>
          <span className="text-sm font-medium text-red-700">Gesamt: {formatEuro(gesamt)}</span>
        </div>
        <BelegTabelle
