    ...
    return (
      <div className="max-w-2xl mx-auto p-6">
        <div className="bg-white border-2 border-dashed border-gray-200 rounded-2xl p-12 text-center">
          ...
          <p className="text-gray-500 text-sm mb-6">Eine Rechnung braucht einen Absender mit Steuernummer und Adresse.</p>
          <Link href="/unternehmen" className="bg-blue-600 text-white px-5 py-2 rounded-lg hover:bg-blue-700 text-sm font-medium">
            Profil anlegen
          </Link>
        </div>
      </div>
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <Link href="/rechnungen" className="text-sm text-gray-500 hover:underline">← Zurück</Link>
      </div>

      {/* Profil + Kunde */}
      <section className="bg-white border rounded-xl p-6 shadow-sm grid grid-cols-2 gap-4">
        <div>
          <label className="block text-xs font-medium text-gray-500 mb-1">Unternehmen (Absender)</label>
      ...
      <section className="bg-white border rounded-xl p-6 shadow-sm space-y-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div className="col-span-2">
            <label className="block text-xs font-medium text-gray-500 mb-1">Straße & Nr.</label>
      ...
      <section className="bg-white border rounded-xl p-6 shadow-sm grid grid-cols-2 gap-4">
        ...
        <div>
          <label className="block text-xs font-medium text-gray-500 mb-1">Leistungsdatum / -zeitraum</label>
        ...
        <div>
          <label className="block text-xs font-medium text-gray-500 mb-1">Zahlungsziel (Tage)</label>
          ...
          <p className="text-xs text-gray-400 mt-1">Fällig am {faellig}</p>
        </div>
        <div className="col-span-2">
      ...
      <section className="bg-white border rounded-xl p-6 shadow-sm space-y-4">
          ...
          {positionen.map((p, i) => (
            <div key={i} className="grid grid-cols-12 gap-2 items-start">
              <div className="col-span-5">
                {i === 0 && <label className="block text-xs font-medium text-gray-500 mb-1">Beschreibung</label>}
              ...
              <div className="col-span-1">
                {i === 0 && <label className="block text-xs font-medium text-gray-500 mb-1">Menge</label>}
              ...
              <div className="col-span-2">
                {i === 0 && <label className="block text-xs font-medium text-gray-500 mb-1">Einzelpreis netto</label>}
              ...
              <div className="col-span-2">
                {i === 0 && <label className="block text-xs font-medium text-gray-500 mb-1">MwSt</label>}
                <select
                  value={p.mwst_satz}
                  disabled={kleinunternehmer}
                  onChange={(e) => setPos(i, { mwst_satz: parseInt(e.target.value) })}
                  className="w-full border rounded-lg px-2 py-2 text-sm disabled:bg-gray-100 disabled:text-gray-400"
                >
              ...
              <div className="col-span-2 flex items-center gap-2">
                {i === 0 && <label className="block text-xs font-medium text-gray-500 mb-1 w-full">&nbsp;</label>}
                ...
                {positionen.length > 1 && (
                  <button onClick={() => setPositionen((arr) => arr.filter((_, idx) => idx !== i))} className="text-red-400 hover:text-red-600 text-lg leading-none">×</button>
                )}
              </div>
            </div>
          ))}
            ...
            {kleinunternehmer ? (
              ...
              <>
                ...
                {summen.mwstGruppen.map((g) => (
                  <div key={g.satz} className="flex justify-between text-gray-600">
        ...
        {kleinunternehmer && (
          <p className="text-xs text-gray-500 text-right">Gemäß § 19 UStG wird keine Umsatzsteuer berechnet.</p>
        )}
      </section>
      ...
      <section className="bg-white border rounded-xl p-6 shadow-sm space-y-4">
        <div>
          <label className="block text-xs font-medium text-gray-500 mb-1">Einleitungstext</label>
        ...
        <div>
          <label className="block text-xs font-medium text-gray-500 mb-1">Fußtext</label>
      ...
      <div className="flex gap-2">
        <button onClick={speichernUndPdf} className="bg-blue-600 text-white px-5 py-2.5 rounded-lg hover:bg-blue-700 text-sm font-semibold">
          Speichern & PDF
        </button>
        <button onClick={vorschauOeffnen} className="bg-white border text-gray-700 px-5 py-2.5 rounded-lg hover:bg-gray-50 text-sm font-medium">
          Vorschau
        </button>
        <button onClick={speichern} className="bg-gray-100 text-gray-700 px-5 py-2.5 rounded-lg hover:bg-gray-200 text-sm font-medium">
          Nur speichern
        </button>
        <Link href="/rechnungen" className="px-5 py-2.5 text-sm text-gray-500 hover:underline self-center">Abbrechen</Link>
      </div>
